...
- **工具返回 token 优化**：查询类工具默认最多返回 500 行（`limit` 参数 1–10000 可调），超限返回 `truncated` 与精确 `total`；`query`/`execute_sql` 支持 `detail_level` 三档分级（summary 只返回概览+示例最省 token / detail 完整 / full 提高上限到 10000）；`query_paginated` 返回 `has_more` 支持续取；输出为紧凑 JSON 并有全局 30K 字符预算兜底

## 快速开始

```bash
# npm 安装（无需 Go 环境，postinstall 自动从 GitHub Releases 下载对应平台二进制）
npm i -g pgsql-server-mcp
